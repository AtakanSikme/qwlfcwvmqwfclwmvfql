...
exports.run = function(client, message, args) {
  ...
  if (!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(`:no_entry: Bu komutu kullanabilmek için "\`Yönetici\`" yetkisine sahip olmalısın.`);
	if(!args[0]) return message.channel.send(new Discord.MessageEmbed()
                                          ...
                                          .setDescription(`
**c!botlist-ayar botekle-kanal #kanal**
**c!botlist-ayar botlog-kanal #kanal**
**c!botlist-ayar başvurugiden-kanal #kanal**
**c!botlist-ayar yetkili @rol**`)
                                          .setColor(message.guild.me.displayColor)
                                          )
  if(args[0] === "botekle-kanal"){
    if(db.has(`bot-ekle_${message.guild.id}`)) return message.channel.send("Bu kanal zaten ayarlanmış.Sıfırlamak için **c!botlist-ayar sıfırla**")
   ...
   if(args[0] === "botlog-kanal"){
     if(db.has(`bot-log_${message.guild.id}`)) return message.channel.send("Bu kanal zaten ayarlanmış.Sıfırlamak için **c!botlist-ayar sıfırla**")
   ...
   if(args[0] === "başvurugiden-kanal"){
     if(db.has(`basvuruk_${message.guild.id}`)) return message.channel.send("Bu kanal zaten ayarlanmış.Sıfırlamak için **c!tlist-ayar sıfırla**")
